...
If(!(test-path $sysinternalsDir)) {
  ...
  Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Sysinternals directory exists, no need to re-install. Exiting."
  exit
}
...
$autorunsPath = "$sysinternalsDir\Autoruns64.exe"
$procmonPath = "$sysinternalsDir\Procmon.exe"
$psexecPath = "$sysinternalsDir\PsExec64.exe"
$procexpPath = "$sysinternalsDir\procexp64.exe"
$sysmonPath = "$sysinternalsDir\Sysmon64.exe"
$sdeletePath = "$sysinternalsDir\Sdelete64.exe"
$tcpviewPath = "$sysinternalsDir\Tcpview.exe"
...
$customConfigPath = "\\host.lan\\Data\\sysmonconfig.xml"
Write-Host "$('[{0:HH:mm}]' -f (Get-Date)) Copying Olaf Hartong's Sysmon config from host..."
Copy-Item -Path $customConfigPath -Destination $sysmonConfigPath
...
# Make the event log channel readable.
